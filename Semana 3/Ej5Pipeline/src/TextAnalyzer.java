...
public class TextAnalyzer {
  ...
  public List<Map.Entry<String, Long>> topN(int n) {
    ...
    return cleanWords().collect(groupingBy(w -> w, counting())).entrySet().stream()
  ...
  public double averageWordLength() {
    ...
    return cleanWords().mapToInt(String::length).average().orElse(0.0);
  }

  public Map<Character, List<String>> wordsByFirstLetter() {
    // TODO: agrupar palabras unicas por primera letra
    return cleanWords().distinct().collect(groupingBy(w -> w.charAt(0)));
  }
  ...
  public static void main(String[] args) {
    ...
    analyzer
        .topN(5)
        .forEach(e -> System.out.printf("  '%s': %d veces%n", e.getKey(), e.getValue()));
