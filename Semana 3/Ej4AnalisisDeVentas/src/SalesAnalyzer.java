...
public class SalesAnalyzer {
  public static void main(String[] args) {
    ...
    Map<String, Double> byCategory =
        sales.stream().collect(groupingBy(Sale::category, summingDouble(Sale::amount)));
    System.out.println("\n=== Ingresos por Categoria ===");
    byCategory.forEach((cat, sum) -> System.out.printf("  %s: $%,.2f%n", cat, sum));
    ...
    Map<String, Double> avgByCategory =
        sales.stream().collect(groupingBy(Sale::category, averagingDouble(Sale::amount)));
    ...
    Map<String, Optional<Sale>> bestPerRegion =
        sales.stream()
            .collect(groupingBy(Sale::region, maxBy(Comparator.comparingDouble(Sale::amount))));
    bestPerRegion.forEach(
        (region, sale) ->
            sale.ifPresent(
                s -> System.out.printf("  %s: %s ($%,.2f)%n", region, s.product(), s.amount())));

    // 7. Conteo por mes
    Map<Integer, Long> byMonth =
        sales.stream().collect(groupingBy(s -> s.date().getMonthValue(), counting()));
    System.out.println("\n=== Ventas por Mes ===");
