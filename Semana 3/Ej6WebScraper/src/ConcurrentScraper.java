...
public class ConcurrentScraper {
  ...
  public List<PageResult> fetchAll(List<String> urls) {
    ...
    List<CompletableFuture<PageResult>> futures =
        urls.stream()
            .map(url -> CompletableFuture.supplyAsync(() -> fetchPage(url), this.executor))
  ...
  public List<PageResult> fetchWithTimeout(List<String> urls, long timeoutMs) {
    List<CompletableFuture<PageResult>> futures =
        urls.stream()
            .map(
                url ->
                    CompletableFuture.supplyAsync(() -> fetchPage(url), executor)
  ...
  public void printReport(List<PageResult> results) {
    ...
    double avg = results.stream().mapToLong(PageResult::responseTimeMs).average().orElse(0);
