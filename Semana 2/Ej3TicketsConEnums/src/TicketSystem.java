...
enum Priority {
  ...
  public String getLabel() {
    ...
    return String.format(
        "%s (Nivel %d, Respuesta: %dh)", name(), this.level, this.responseTimeHours);
  }
}
...
class Ticket {
  ...
  public void transitionTo(TicketStatus newStatus) {
    ...
    if (this.status.canTransitionTo(newStatus)) {
      System.out.printf("Ticket %s: %s -> %s%n", this.id, this.status, newStatus);
...
public class TicketSystem {
  public static void main(String[] args) {
    ...
    for (Ticket t : tickets) {
      if (urgentes.contains(t.getPriority())) System.out.println(t.toString());
    }
  }
}
